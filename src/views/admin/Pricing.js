...
export default function Pricing() {
  ...
  return (
    <>
      <div
        className=" bg-blueGray-700 h-full py-16 antialiased bg-primary-very-light font-sans"
        style={{ marginTop: 150 }}
      >
        <div
          id="modal"
        ...
        <section className="flex flex-col  lg:flex-row items-start items-center lg:justify-center w-full w-full lg:px-10 py-12">
          {packs.map((pack) => {
            if (pack._id === user.pack._id) {
              return (
                <article
                  className="w-full  bg-white lg:w-custom w-4/5 mb-10 px-6 py-16 lg:-mt-6 text-white text-center rounded-lg"
                  style={{
              ...
              return (
                <article
                  className="w-full bg-white w-4/5 lg:w-custom mb-10 lg:px-4 px-6 py-10 text-center text-primary-dark rounded-lg"
                  key={`pricing_${pack._id}`}
                >
                  <h5 className="font-bold text-base">{pack.name}</h5>
